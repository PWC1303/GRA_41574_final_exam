...
from utils.alpha_tuner import alpha_tuner
...
def main():
    ...
    X_temp,X_te,y_temp,y_te = train_test_split(X,y,test_size=0.3,random_state=33)
    X_tr,X_val,y_tr,y_val= train_test_split(X_temp,y_temp,test_size=0.3,random_state=33)
    ...
    if args.model == "logm_l2":
        ...
        joblib.dump(model, f"tuning_results/models/{args.model}.pkl")
        print(f"params, cv_pr_auc and model.pkl saved for {args.model}")
        alpha_opt = alpha_tuner(args.model,model,X_val,y_val)
        print(alpha_opt)
